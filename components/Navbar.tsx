            ...
            <div className={`fixed inset-0 bg-white dark:bg-brand-navy z-40 transition-transform duration-300 ease-in-out pt-24 px-6 md:hidden overflow-y-auto ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <div className="flex flex-col space-y-6">
                    ...
                    <div className="flex flex-col space-y-3 pb-2 border-b border-slate-100 dark:border-slate-800">
                        ...
                        <div className="pl-4 flex flex-col space-y-3">
                            <Link href="/koleksiyon#hooks" onClick={() => setIsMobileMenuOpen(false)} className={`text-sm ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>{t('footer_col_hooks')}</Link>
                            <Link href="/koleksiyon#fenders" onClick={() => setIsMobileMenuOpen(false)} className={`text-sm ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>{t('cat_fenders')}</Link>
                            <Link href="/koleksiyon#lines" onClick={() => setIsMobileMenuOpen(false)} className={`text-sm ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>{t('cat_fender_lines')}</Link>
                            <Link href="/koleksiyon#covers" onClick={() => setIsMobileMenuOpen(false)} className={`text-sm ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>{t('cat_fender_covers')}</Link>
                            <Link href="/koleksiyon#cleats" onClick={() => setIsMobileMenuOpen(false)} className={`text-sm ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>{t('footer_col_cleats')}</Link>
                            <Link href="/koleksiyon#ladders" onClick={() => setIsMobileMenuOpen(false)} className={`text-sm ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>{t('footer_col_ladders')}</Link>
                            <Link href="/koleksiyon#dinghy-hangers" onClick={() => setIsMobileMenuOpen(false)} className={`text-sm ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>{t('cat_dinghy_hangers')}</Link>
                        </div>
                    </div>
                    ...
                    <Link href="/teklif" onClick={() => setIsMobileMenuOpen(false)} className={`text-lg uppercase tracking-widest font-medium border-b pb-2 text-brand-gold`}>{t('nav_quote')}</Link>
                            ...
                            <div className="flex gap-3">
                                {Object.entries(flags).map(([code, flag]) => (
                                    <button
                                        ...
                                        onClick={() => {
                                            setLanguage(code as Language);
                                            // Don't close menu immediately, user might want to see change
                                        }}
                                        className={`px-4 py-2 rounded-sm border text-sm font-bold flex items-center gap-2 transition-colors ${language === code ? 'border-brand-gold bg-brand-gold/10 text-brand-gold' : (darkMode ? 'border-slate-700 text-slate-400' : 'border-slate-200 text-slate-600')}`}
                                    >
                                        <span className="text-lg">{flag}</span>
                                        {code}
                                    </button>
